...
namespace AutosarBCM.Config
{
    ...
    public class ASResponse
    {
        ...
        public ASResponse(byte[] data)
        {
            ...
            ControlInfo = ASApp.Configuration.Controls.Where(x => x.Address == BitConverter.ToUInt16(data.Skip(1).Take(2).Reverse().ToArray(), 0)).FirstOrDefault();
            foreach (var pInfo in ControlInfo.Responses.Where(a => a.ServiceID == data[0]).First().Payloads)
