...
namespace AutosarBCM.Core
{
                ...
                else //Payload match
                {
                    ...
                    else
                    {
                        if (resultPayload.TypeName == "DID_PWM")
                        {
                            byte[] pwmBytes;
                            if (isOpen)
                                pwmBytes = BitConverter.GetBytes((ushort)ASContext.Configuration.EnvironmentalTest.Envrironments.First(x => x.Name == EnvironmentalTest.CurrentEnvironment).EnvironmentalConfig.PWMDutyOpenValue).Reverse().ToArray();
                            else
                                pwmBytes = BitConverter.GetBytes((ushort)ASContext.Configuration.EnvironmentalTest.Envrironments.First(x => x.Name == EnvironmentalTest.CurrentEnvironment).EnvironmentalConfig.PWMDutyCloseValue).Reverse().ToArray();
        ...
        internal static ConfigurationInfo Parse(string filePath)
        {
            ...
            var environmentalTest = doc.Descendants("EnvironmentalTest")
                .Select(t => new EnvironmentalTest
                {
                    ...
                    Envrironments = t.Element("Environments").Elements("Environment")
    ...
    public class EnvironmentalTest
    {
        ...
        public List<Environment> Envrironments { get; set; }
    }
