    ...
    public class ASResponse
    {
        ...
        public static ASResponse Parse(byte[] data)
        {
            ...
            foreach (var pInfo in response.ControlInfo?.GetPayloads(response.ServiceInfo.ResponseID))
            {
                ...
                pInfo.ResponseColors = payloadDef?.ResponseColors;
                response.Payloads.Add((Payload)Activator.CreateInstance(Type.GetType($"AutosarBCM.Core.{pInfo.TypeName}"), new object[] { pInfo, data }));
